...
public class EduAppServer {
  ...
  public EduAppServer() {   // **변경사항**
    // 설정파일의 정보를 보고 클래스를 찾아 객체를 생성한다.
    appContext = new ClassPathXmlApplicationContext(
        new String[] {"bitcamp/java89/ems/server/application-context.xml"}); // 스프링 설정파일 경로정보
    
    //스프링 IoC 컨테이너에 들어있는 객체들의 이름을 가져온다.
    String[] names = appContext.getBeanDefinitionNames(); //Return the names of all beans defined in this factory.
